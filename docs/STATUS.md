...
- **Known follow-ups (non-blocking)**:
  ...
  - The deploy role's SSM grant was wider than "read": it held `ssm:GetParameter*`, `ssm:PutParameter` **and**
    ...
    write. ✅ **Fixed on branch `fix/deploy-role-least-privilege`** (not merged, not deployed): those three actions
    are now scoped to `/five-crowns/*` plus the `/sst/*` paths SST itself needs (passphrase, bootstrap).
    ⚠️ **Applying it needs `scripts/aws-bootstrap.sh` re-run** — a merge alone changes nothing in AWS — and the
    first deploy after it should be watched. Still open on that role: `kms:Encrypt`/`kms:Decrypt` on `*`,
    `ssm:DescribeParameters` on `*` (SSM supports no resource scope for it), and the unconditioned `iam:PassRole`
    on `five-crowns-*`.
