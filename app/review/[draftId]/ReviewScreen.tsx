...
export function ReviewScreen({ draftId }: { draftId: string }) {
  ...
  // PRD criterion 322: flips true once the in-flight `POST /api/games` has
  // been running for `SAVE_LONG_WAIT_DELAY_MS` — swaps `SaveBar`'s helper
  // line to `SAVE_BUTTON_LONG_WAIT_MESSAGE`. Reset (and its timer cleared)
  // the instant the fetch resolves, whichever way.
  const [saveLongWait, setSaveLongWait] = useState(false);
  ...
  // prompt — `saveStatus` only becomes `"saving"` inside `handleSave` (never
  // during ordinary autosave), so this never fires for the debounced
  // `putDraft` PUT. Adding/removing the listener as an effect keyed on
  // `saveStatus`, rather than inside `handleSave` itself, means it's also
  // guaranteed gone the instant `saveStatus` moves to `"error"` or back to
  // `"idle"`, on every path — no separate cleanup call to remember.
  useEffect(() => {
  ...
  async function handleSave() {
    ...
    try {
      ...
      // PRD criterion 322: the fetch has resolved (success or failure) — the
      // long-wait message (if it already fired) and its pending timer (if it
      // hasn't) are both cleared here, on every path, including the redirect
      // above (a `return` inside `try` still runs `finally`).
