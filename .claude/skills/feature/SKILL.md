...
## 4. Build (frontend-developer and/or backend-developer agents)
- Backend first if the feature spans both (frontend needs the API shape).
- Developers follow `docs/ARCHITECTURE.md` and `docs/DESIGN-SYSTEM.md`.

## 5. Verify (qa-engineer agent)
QA tests each acceptance criterion by exercising the feature, adds automated tests, and reports pass/fail. A failed criterion goes back to step 4 — max two round-trips before escalating to the founder.

## 6. Review & PR
- Run the built-in `/code-review` skill on the diff; fix confirmed findings.
- If the feature touches auth, payments, user data, or new public endpoints: launch the **security-reviewer** agent first and fix blockers.
